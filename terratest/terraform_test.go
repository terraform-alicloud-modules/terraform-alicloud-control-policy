...
func TestTerraformBasicExampleNew(t *testing.T) {
	t.Parallel()
	controlPolicyName := fmt.Sprintf("tf-test-%d", random.Random(100, 1000))
	...
	thisControlPolicyName := terraform.Output(t, terraformOptions, "this_control_policy_name")

	// Verify we're getting back the outputs we expect
	assert.Equal(t, thisControlPolicyName, controlPolicyName)
}
